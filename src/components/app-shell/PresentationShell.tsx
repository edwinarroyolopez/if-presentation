...
export function PresentationShell({ children }: { children: React.ReactNode }) {
  ...
  const sidebar = <><BrandBlock /><Progress part={progress.item.part} percent={progress.percent} totalParts={progress.totalParts} /><Nav active={normalized} onNavigate={() => setOpen(false)} totalParts={progress.totalParts} /><div className="sidebar-foot">Teclado: Tab para navegar · Escape cierra menú móvil</div></>;

  return <div className="presentation-app"><aside aria-label="Navegación principal" className="presentation-sidebar">{sidebar}</aside><div className="workspace"><header className="topbar"><button ref={openerRef} className="icon-btn mobile-menu" aria-label="Abrir menú" onClick={() => setOpen(true)}><Icon name="menu" /></button><div className="crumb"><small>InflightOS / Evaluación estratégica</small><strong>{progress.item.shortLabel}</strong></div><div className="top-actions"><span className="counter">{progress.item.part} de {progress.totalParts}</span><FullscreenToggle /></div></header><main className="content-stage"><div className="slide-frame"><div className="slide-pad">{children}</div></div><PreviousNext previous={progress.previous} next={progress.next} /></main></div><div className={open ? "drawer-backdrop open" : "drawer-backdrop"} aria-hidden={!open} onMouseDown={(event) => { if (event.target === event.currentTarget) setOpen(false); }}><aside aria-label="Menú móvil" className="mobile-drawer"><button ref={closeRef} className="icon-btn" aria-label="Cerrar menú" onClick={() => { setOpen(false); openerRef.current?.focus(); }} style={{ margin: 14 }}><Icon name="close" /></button>{sidebar}</aside></div></div>;
}

function Progress({ part, percent, totalParts }: { part: number; percent: number; totalParts: number }) {
  return <div className="progress-panel"><header><b>Progreso</b><span>{part} de {totalParts}</span></header><div className="progress-track" aria-hidden="true"><i style={{ width: `${percent}%` }} /></div></div>;
}

function Nav({ active, onNavigate, totalParts }: { active: string; onNavigate?: () => void; totalParts: number }) {
  return <nav className="part-nav" aria-label="Partes de la presentación"><div className="nav-label">Índice</div>{getNavigationItems().map((item) => { const current = isRouteActive(item.route, active); const extra = item.icon === "brain"; return <Link aria-current={current ? "page" : undefined} className={current ? "part-link active" : "part-link"} href={item.route} key={item.id} onClick={onNavigate}><span className="n">{extra ? <Icon name="brain" size={16} /> : item.part === 0 ? "00" : String(item.part).padStart(2, "0")}</span><span><b>{item.label}</b><small>{extra ? "Graphify Brain" : item.part === 0 ? "Preámbulo" : `Parte ${item.part} de ${totalParts}`}</small></span></Link>; })}</nav>;
}
