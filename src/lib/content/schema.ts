...
export const roadmapSchema = z.object({ phases: z.array(z.object({ days: z.union([z.literal(30), z.literal(90), z.literal(180), z.literal(365)]), title: z.string().min(1), deliverables: z.array(z.string().min(1)).min(1), risks: z.array(z.string().min(1)).min(1), exitCriteria: z.array(z.string().min(1)).min(1) }).strict()).length(4), dependencies: z.array(itemSchema).min(1), hiring: z.array(itemSchema).min(1) }).strict().refine((value) => value.phases.map((phase) => phase.days).join(",") === "30,90,180,365", "Roadmap must contain 30, 90, 180 and 365 days in order");
...
export const presentationSchema = z.object({ navigation: navigationSchema, home: homeSchema, architecture: architectureSchema, roadmap: roadmapSchema, integration: integrationSchema, dataModeling: dataModelingSchema, aiStrategy: aiStrategySchema, executiveScenario: executiveScenarioSchema }).strict();
